...
@section('title', ucfirst($category) . ' Articles | Awais Ahmad')
@section('description', 'Browse all blog posts in the ' . $category . ' category covering web development, design, and technology.')

@section('content')
    <section class="relative py-20">
        <div class="mx-auto max-w-6xl">
            <div class="mb-14 max-w-3xl">
                <p class="text-sm uppercase tracking-[0.28em] text-cyan-300/70 gsap-reveal">Category</p>
                ...
                <p class="mt-5 max-w-2xl text-pretty text-lg leading-8 text-slate-300 gsap-reveal" data-gsap-delay="0.08">
                    @if($blogs->count())
                        {{ $blogs->total() }} article{{ $blogs->total() !== 1 ? 's' : '' }} in the {{ $category }} category.
                    @else
            ...
            <nav class="mb-8 gsap-reveal" data-gsap-delay="0.16">
                <div class="flex items-center gap-2 text-sm text-slate-400">
                    ...
                    <i class="fa-solid fa-chevron-right text-xs"></i>
                    <span class="text-white">{{ ucfirst($category) }}</span>
                </div>
            </nav>

            <!-- Category Filter -->
            <div class="glass-card mb-12 rounded-[1.75rem] border border-white/10 bg-slate-950/55 p-6 gsap-reveal" data-gsap-delay="0.24">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('blog.index') }}"
                       class="rounded-2xl px-4 py-2 text-sm font-medium transition-colors border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10">
                        All Posts
                    </a>
                    @foreach($categories ?? [] as $cat)
                        <a href="{{ route('blog.category', $cat) }}"
                           class="rounded-2xl px-4 py-2 text-sm font-medium transition-colors {{ $cat === $category ? 'bg-cyan-400 text-slate-950' : 'border border-white/10 bg-white/5 text-slate-200 hover:bg-white/10' }}">
                            {{ $cat }}
                        </a>
                    @endforeach
                </div>
            ...
            @if($blogs->count())
                ...
                <!-- Pagination -->
                <div class="mt-12 flex justify-center gsap-reveal" data-gsap-delay="0.32">
                    {{ $blogs->links() }}
                </div>
            @else
                <!-- No Posts Found -->
                <div class="glass-card rounded-[1.75rem] border border-white/10 bg-slate-950/55 p-12 text-center gsap-reveal" data-gsap-delay="0.24">
                    <i class="fa-solid fa-folder-open text-6xl text-slate-600 mb-6"></i>
                    <h3 class="text-2xl font-orbitron text-white mb-4">No Articles Found</h3>
                    <p class="text-slate-400 mb-6">There are no articles in the {{ $category }} category yet.</p>
    ...
    @endif
            ...
            @empty
                <div class="col-span-full text-center py-12">
                    ...
                    <h3 class="text-2xl font-bold text-white mb-2">No Posts in {{ ucfirst($category) }}</h3>
                    <p class="text-cyan-200/80 mb-6">No posts found in this category yet.</p>
                    <a href="{{ route('blog.index') }}" 
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
                            ...
                            <div class="space-y-2">
                                @foreach($categories as $cat)
                                    <a href="{{ route('blog.category', $cat) }}" 
                                       class="block p-3 rounded-xl {{ $cat === $category ? 'bg-cyan-500/20 border border-cyan-500/30' : 'bg-white/5 hover:bg-white/10' }} transition-colors">
                                        <span class="text-white font-medium">{{ ucfirst($cat) }}</span>
                                        @if($cat === $category)
                                            <i class="fas fa-check text-cyan-400 ml-2"></i>
                                        @endif
                                    </a>
                                @endforeach
                            </div>
