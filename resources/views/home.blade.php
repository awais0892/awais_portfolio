...
@section('content')
    ...
    <section id="home" class="min-h-screen flex items-center">
        <div class="text-center md:text-left w-full">
            ...
            <h1 class="text-4xl md:text-7xl font-orbitron text-white mt-2 mb-4 leading-tight gsap-reveal hero-text" data-gsap-delay="0.1">
            ...
            <div class="gsap-reveal hero-text" data-gsap-delay="0.3">
                <a href="{{ route('projects') }}" class="bg-cyan-500 hover:bg-cyan-400 text-black font-bold py-3 px-6 rounded-lg transition-colors mr-4 glow-button">
                    View Projects
                </a>
                <a href="{{ route('cv.download') }}" class="border-2 border-cyan-500 hover:bg-cyan-500/20 text-white font-bold py-3 px-6 rounded-lg transition-all">
                    Download CV
                </a>
            </div>
        </div>
    ...
    <section id="about" class="py-20">
        ...
        <div class="flex flex-col md:flex-row items-center gap-12 glass-card p-8 rounded-lg">
            <div class="md:w-1/3 gsap-reveal">
                <img src="{{ asset('storage/' . ($settings['profile_image'] ?? 'awais.jpg')) }}" alt="Awais Ahmad" class="rounded-full mx-auto shadow-lg w-64 h-64 md:w-full md:h-auto object-cover border-4 border-cyan-500/50">
            </div>
            <div class="md:w-2/3 gsap-reveal" data-gsap-delay="0.1">
                ...
                <div class="flex gap-4">
                    <a href="{{ $settings['https://www.linkedin.com/in/awaisahmads/'] ?? 'https://www.linkedin.com/in/awaisahmads/' }}" target="_blank" class="bg-cyan-500 hover:bg-cyan-400 text-black font-bold py-3 px-6 rounded-lg transition-colors glow-button">
                        LinkedIn Profile
                    </a>
                    <a href="{{ $settings['https://github.com/awais0892'] ?? 'https://github.com/awais0892' }}" target="_blank" class="border-2 border-cyan-500 hover:bg-cyan-500/20 text-white font-bold py-3 px-6 rounded-lg transition-all">
                        GitHub Profile
                    </a>
                </div>
            </div>
    ...
    <section id="projects" class="py-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($projects as $project)
                <div class="glass-card rounded-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 gsap-reveal" data-gsap-delay="{{ $loop->index * 0.1 }}">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex gap-4">
                            @if($project->live_url)
                                <a href="{{ $project->live_url }}" target="_blank" class="text-cyan-400 hover:underline font-semibold">Live Demo &rarr;</a>
                            @endif
                            @if($project->github_url)
                                <a href="{{ $project->github_url }}" target="_blank" class="text-cyan-400 hover:underline font-semibold">Source Code &rarr;</a>
                            @endif
                            <a href="{{ route('project.show', $project->slug) }}" class="text-cyan-400 hover:underline font-semibold">View Details &rarr;</a>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="text-center mt-8">
            <a href="{{ route('projects') }}" class="bg-cyan-500 hover:bg-cyan-400 text-black font-bold py-3 px-6 rounded-lg transition-colors glow-button">
                View All Projects
            </a>
        </div>
    </section>
        ...
        <div class="max-w-6xl mx-auto glass-card p-8 rounded-lg">
            <div class="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-8 gap-8">
                @foreach($skills as $skill)
                    <div class="flex flex-col items-center gap-2 gsap-reveal" title="{{ $skill->name }} - {{ $skill->proficiency }}%">
                ...
                <div class="glass-card p-8 rounded-lg exp-card gsap-reveal" data-gsap-delay="{{ $loop->index * 0.1 }}">
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-2">
                        <div>
                            ...
                            <p class="text-cyan-400 text-lg">{{ $edu->institution }}@if($edu->location), {{ $edu->location }}@endif</p>
                        </div>
                        <span class="text-gray-400 bg-cyan-900/30 px-3 py-1 rounded-full text-sm mt-2 md:mt-0">{{ $edu->duration }}</span>
                    </div>
                    @if($edu->grade)
    ...
    <section id="contact" class="py-20">
        ...
        <p class="text-gray-300 max-w-2xl mx-auto mb-8 text-lg text-center gsap-reveal" data-gsap-delay="0.1">
            I'm currently open to new opportunities. Whether you'd like to discuss a potential collaboration or just want to say hello, feel free to reach out!
        </p>
            ...
            <form id="contact-form" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" required class="w-full p-3 rounded-lg form-input transition-all duration-300" placeholder="e.g., Jane Doe">
                    </div>
                    <div>
                        <label for="email" class="block text-left text-cyan-400 font-semibold mb-2">Your Email</label>
                        <input type="email" id="email" name="email" required class="w-full p-3 rounded-lg form-input transition-all duration-300" placeholder="e.g., jane@example.com">
                    </div>
                </div>
                <div>
                    <label for="subject" class="block text-left text-cyan-400 font-semibold mb-2">Subject</label>
                    <input type="text" id="subject" name="subject" required class="w-full p-3 rounded-lg form-input transition-all duration-300" placeholder="e.g., Project Opportunity">
                </div>
                <div>
                    <label for="message" class="block text-left text-cyan-400 font-semibold mb-2">Message</label>
                    <textarea id="message" name="message" rows="6" required class="w-full p-3 rounded-lg form-input transition-all duration-300" placeholder="Your message here..."></textarea>
                </div>
                <div class="text-center">
                    <button type="submit" class="bg-cyan-500 hover:bg-cyan-400 text-black font-bold py-3 px-8 rounded-lg transition-colors text-lg glow-button">
                        Send Message
                    </button>
                </div>
            </form>
