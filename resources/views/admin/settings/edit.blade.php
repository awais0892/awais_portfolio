                ...
                <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-6 lg:space-y-0">
                    <div class="space-y-4">
                        <a href="{{ route('admin.settings.index') }}"
                            ...
                            <div
                                class="w-8 h-8 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-full flex items-center justify-center border border-cyan-400/30 mr-3 group-hover:scale-110 transition-transform duration-300">
                        ...
                        <div>
                            <h1
                                class="text-5xl font-black bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 bg-clip-text text-transparent mb-4 tracking-tight">
                                EDIT SETTING
                            </h1>
                    ...
                    <div class="flex space-x-4">
                        <a href="{{ route('admin.settings.show', $setting) }}"
                            ...
                            <span class="relative z-10 font-semibold">View Setting</span>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl">
                <form method="POST" action="{{ route('admin.settings.update', $setting) }}" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label for="key"
                                class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-3">Setting Key
                                *</label>
                        ...
                        <div>
                            <label for="type"
                                class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-3">Setting Type
                                *</label>
                            <select id="type" name="type" required
                                ...
                                <option value="text" class="bg-slate-800" {{ old('type', $setting->type) == 'text' ? 'selected' : '' }}>Text</option>
                                <option value="textarea" class="bg-slate-800" {{ old('type', $setting->type) == 'textarea' ? 'selected' : '' }}>Textarea</option>
                                <option value="image" class="bg-slate-800" {{ old('type', $setting->type) == 'image' ? 'selected' : '' }}>Image</option>
                                <option value="json" class="bg-slate-800" {{ old('type', $setting->type) == 'json' ? 'selected' : '' }}>JSON</option>
                                <option value="boolean" class="bg-slate-800" {{ old('type', $setting->type) == 'boolean' ? 'selected' : '' }}>Boolean</option>
                                <option value="number" class="bg-slate-800" {{ old('type', $setting->type) == 'number' ? 'selected' : '' }}>Number</option>
                            </select>
                            @error('type')
                                <p class="text-red-400 text-sm mt-2 flex items-center">
                                    <i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label for="group"
                                class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-3">Group *</label>
                            <select id="group" name="group" required
                                class="js-choice w-full bg-white/10 border border-white/20 rounded-xl px-6 py-4 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300 text-lg">
                                <option value="general" class="bg-slate-800" {{ old('group', $setting->group) == 'general' ? 'selected' : '' }}>General</option>
                                <option value="social" class="bg-slate-800" {{ old('group', $setting->group) == 'social' ? 'selected' : '' }}>Social</option>
                                <option value="contact" class="bg-slate-800" {{ old('group', $setting->group) == 'contact' ? 'selected' : '' }}>Contact</option>
                                <option value="seo" class="bg-slate-800" {{ old('group', $setting->group) == 'seo' ? 'selected' : '' }}>SEO</option>
                                <option value="appearance" class="bg-slate-800" {{ old('group', $setting->group) == 'appearance' ? 'selected' : '' }}>Appearance</option>
                                <option value="security" class="bg-slate-800" {{ old('group', $setting->group) == 'security' ? 'selected' : '' }}>Security</option>
                            </select>
                            @error('group')
                                <p class="text-red-400 text-sm mt-2 flex items-center">
                                    <i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}
                        ...
                        <div>
                            <label for="label"
                                class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-3">Display
                                Label</label>
                            <input type="text" id="label" name="label" value="{{ old('label', $setting->label) }}"
                                class="w-full bg-white/10 border border-white/20 rounded-xl px-6 py-4 text-white placeholder-cyan-300/50 focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300 text-lg"
                    ...
                    <div>
                        <label for="description"
                            class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-3">Description</label>
                    ...
                    @if($setting->type === 'image')
                        <div class="p-6 backdrop-blur-sm bg-white/5 border border-white/10 rounded-2xl">
                            ...
                            <div class="flex items-center space-x-6">
                                ...
                                @else
                                    <div
                                        class="w-40 h-32 bg-white/5 border-2 border-dashed border-white/20 rounded-2xl flex items-center justify-center">
                                        <span class="text-cyan-300/50">No image</span>
                                    </div>
                                @endif
                                <div class="text-cyan-300/80">
                                    <p class="text-lg font-semibold">Current setting image</p>
                    ...
                    <div class="flex flex-col sm:flex-row justify-end space-y-4 sm:space-y-0 sm:space-x-4">
                        ...
                        <button type="submit"
                            ...
                            <span class="relative z-10 font-semibold text-lg">Update Setting</span>
                        </button>
                    </div>
                </form>
            </div>
