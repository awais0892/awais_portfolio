...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl">
            <form method="POST" action="{{ route('admin.settings.update', $setting) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div id="valueField">
                    ...
                    @else
                        <input type="{{ $setting->type === 'number' ? 'number' : 'text' }}" id="value" name="value" 
                               value="{{ old('value', $setting->type === 'json' ? json_encode($setting->value) : $setting->value) }}" required 
                               class="w-full bg-white/10 border border-white/20 rounded-xl px-6 py-4 text-white placeholder-cyan-300/50 focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300 text-lg"
                ...
                @if($setting->type === 'image')
                    <div class="p-6 backdrop-blur-sm bg-white/5 border border-white/10 rounded-2xl">
                        ...
                        <div class="flex items-center space-x-6">
                            @if($setting->value)
                                <img src="{{ asset('storage/' . $setting->value) }}" alt="{{ $setting->label ?? $setting->key }}" 
                                     class="w-40 h-32 object-cover rounded-2xl border-2 border-white/20 hover:border-cyan-400/50 transition-all duration-300">
                            @else
                                <div class="w-40 h-32 bg-white/5 border-2 border-dashed border-white/20 rounded-2xl flex items-center justify-center">
                    ...
                    <div>
                        <label for="image" class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-3">Upload New Image</label>
