        ...
        <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-8 mb-8 shadow-2xl">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select id="statusFilter" class="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300">
                ...
                <div class="space-y-2">
                    ...
                    <select id="featuredFilter" class="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300">
                ...
                <div class="space-y-2">
                    ...
                    <select id="perPageFilter" class="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300">
                ...
                <table class="min-w-full divide-y divide-white/10">
                    <thead class="bg-white/5 backdrop-blur-sm">
                        <tr>
                            <th class="px-8 py-6 text-left">
                                <input type="checkbox" id="selectAllCheckbox" class="w-5 h-5 rounded-lg border-white/30 text-cyan-500 focus:ring-cyan-400 bg-white/10">
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="id">
                                <div class="flex items-center space-x-2">
                                    <span>ID</span>
                                    <i class="fas fa-sort text-cyan-400/50"></i>
                                </div>
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="title">
                                <div class="flex items-center space-x-2">
                                    <span>Project</span>
                                    <i class="fas fa-sort text-cyan-400/50"></i>
                                </div>
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="technologies">
                                <span>Technologies</span>
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="featured">
                                <div class="flex items-center space-x-2">
                                    <span>Featured</span>
                                    <i class="fas fa-sort text-cyan-400/50"></i>
                                </div>
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="status">
                                <div class="flex items-center space-x-2">
                                    <span>Status</span>
                                    <i class="fas fa-sort text-cyan-400/50"></i>
                                </div>
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="order">
                                <div class="flex items-center space-x-2">
                                    <span>Order</span>
                                    <i class="fas fa-sort text-cyan-400/50"></i>
                                </div>
                            </th>
                            <th class="px-8 py-6 text-left text-sm font-bold text-cyan-300 uppercase tracking-wider cursor-pointer hover:bg-white/5 transition-colors duration-300" data-sort="created_at">
                                <div class="flex items-center space-x-2">
                                    <span>Created</span>
                                    <i class="fas fa-sort text-cyan-400/50"></i>
                                </div>
                    ...
                    <tbody class="divide-y divide-white/10" id="projectsTableBody">
                        ...
                        <tr class="hover:bg-white/5 transition-all duration-300 group project-row">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center space-x-4">
                                    <div class="relative group">
                                        <div class="w-16 h-12 rounded-xl overflow-hidden border-2 border-white/20 group-hover:border-cyan-400 transition-all duration-300">
                                    ...
                                    <div>
                                        <div class="text-lg font-bold text-white group-hover:text-cyan-300 transition-colors duration-300">{{ $project->title }}</div>
                                        <div class="text-sm text-cyan-300/70 max-w-xs">{{ Str::limit($project->description, 60) }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                @if($project->technologies && count($project->technologies) > 0)
                                    <div class="flex flex-wrap gap-2">
                                        @foreach(array_slice($project->technologies, 0, 3) as $tech)
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 border border-cyan-400/30 backdrop-blur-sm">
                                                {{ $tech }}
                                            </span>
                                        @endforeach
                                        @if(count($project->technologies) > 3)
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-300 border border-purple-400/30 backdrop-blur-sm">
                            ...
                            <td class="px-8 py-6">
                                @if($project->featured)
                                    <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-yellow-400/20 to-orange-400/20 text-yellow-300 border border-yellow-400/30 backdrop-blur-sm">
                                        <i class="fas fa-star mr-2 text-yellow-400"></i>Featured
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-gray-400/20 to-gray-500/20 text-gray-300 border border-gray-400/30 backdrop-blur-sm">
                                        Regular
                                    </span>
                                @endif
                            </td>
                            <td class="px-8 py-6">
                                <button onclick="toggleProjectStatus({{ $project->id }}, {{ $project->is_active ? 'true' : 'false' }})" 
                                    ...
                                    <div class="absolute inset-0 rounded-full bg-gradient-to-r from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                            ...
                            <td class="px-8 py-6 text-right">
                                <div class="flex items-center justify-end space-x-3">
                                    <a href="{{ route('admin.projects.show', $project) }}" 
                                       class="group relative p-3 rounded-xl bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 border border-cyan-400/30 hover:from-cyan-500/30 hover:to-blue-500/30 transition-all duration-300 hover:scale-110" title="View">
                                    ...
                                    <a href="{{ route('admin.projects.edit', $project) }}" 
                                       class="group relative p-3 rounded-xl bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-blue-300 border border-blue-400/30 hover:from-blue-500/30 hover:to-purple-500/30 transition-all duration-300 hover:scale-110" title="Edit">
                                        <div class="absolute inset-0 rounded-xl bg-gradient-to-r from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                        <i class="fas fa-edit relative z-10"></i>
                                    </a>
                                    <button onclick="deleteProject({{ $project->id }}, '{{ $project->title }}')" 
                                            class="group relative p-3 rounded-xl bg-gradient-to-r from-red-500/20 to-pink-500/20 text-red-300 border border-red-400/30 hover:from-red-500/30 hover:to-pink-500/30 transition-all duration-300 hover:scale-110" title="Delete">
                                        <div class="absolute inset-0 rounded-xl bg-gradient-to-r from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                        ...
                        <tr>
                            <td colspan="8" class="px-8 py-16 text-center">
                                <div class="flex flex-col items-center space-y-4">
                                    <div class="w-20 h-20 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 rounded-full flex items-center justify-center border border-cyan-400/30">
        ...
        <div class="mt-8 backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-6">
                    <button id="selectAllBtn" class="group relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 text-cyan-300 border border-cyan-400/30 rounded-xl hover:from-cyan-500/30 hover:to-blue-500/30 transition-all duration-300 hover:scale-105">
                        <div class="absolute inset-0 rounded-xl bg-gradient-to-r from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                        <i class="fas fa-check-square mr-3 relative z-10"></i>
                        <span class="relative z-10 font-semibold">Select All</span>
                    </button>
                    <button id="bulkDeleteBtn" class="group relative inline-flex items-center px-6 py-3 bg-gradient-to-r from-red-500/20 to-pink-500/20 text-red-300 border border-red-400/30 rounded-xl hover:from-red-500/30 hover:to-pink-500/30 transition-all duration-300 hover:scale-105 hidden">
                        <div class="absolute inset-0 rounded-xl bg-gradient-to-r from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
