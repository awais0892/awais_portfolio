        ...
        <div class="backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-6 shadow-2xl mb-8">
            <form method="GET" action="{{ route('admin.blogs.index') }}" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <!-- Search -->
                    <div>
                        <label class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-2">Search</label>
                    ...
                    <!-- Status Filter -->
                    <div>
                        <label class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-2">Status</label>
                    ...
                    <!-- Category Filter -->
                    <div>
                        <label class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-2">Category</label>
                        <select name="category" class="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300">
                    ...
                    <!-- Sort -->
                    <div>
                        <label class="block text-sm font-bold text-cyan-300 uppercase tracking-wider mb-2">Sort By</label>
                        <select name="sort_by" class="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300">
            ...
            <form id="bulkActionForm" method="POST" action="{{ route('admin.blogs.bulk-action') }}" class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
                    <div class="flex items-center space-x-4">
                        <select id="bulkAction" name="action" class="bg-white/10 border border-white/20 rounded-xl px-4 py-2 text-white focus:ring-2 focus:ring-cyan-400 focus:border-transparent backdrop-blur-sm transition-all duration-300">
                    ...
                    <thead class="bg-white/5">
                        <tr>
                            <th class="px-6 py-4 text-left">
                                <input type="checkbox" id="selectAll" class="w-5 h-5 rounded-lg border-white/30 text-cyan-500 focus:ring-cyan-400 bg-white/10">
                            </th>
                            ...
                            <th class="px-6 py-4 text-left text-cyan-300 font-bold uppercase tracking-wider">Category</th>
                            ...
                            <th class="px-6 py-4 text-left text-cyan-300 font-bold uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="group hover:bg-white/5 transition-all duration-300">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div>
                                            <h3 class="text-white font-semibold text-lg group-hover:text-cyan-300 transition-colors duration-300">
                                                {{ $blog->title }}
                                            </h3>
                                            ...
                                            <div class="flex items-center space-x-2 mt-2">
                                                @if($blog->tags)
                                                    @foreach(array_slice($blog->tags, 0, 2) as $tag)
                                                        <span class="px-2 py-1 bg-cyan-500/20 text-cyan-300 text-xs rounded-lg">{{ $tag }}</span>
                                                    @endforeach
                                                    @if(count($blog->tags) > 2)
                                                        <span class="text-cyan-300/50 text-xs">+{{ count($blog->tags) - 2 }} more</span>
                                                    @endif
                                                @endif
                                            </div>
                                        </div>
                                ...
                                <td class="px-6 py-4">
                                    @if($blog->category)
                                        <span class="px-3 py-1 bg-blue-500/20 text-blue-300 text-sm rounded-lg">{{ $blog->category }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button onclick="toggleStatus({{ $blog->id }}, '{{ $blog->status }}')" 
                                                class="text-green-400 hover:text-green-300 transition-colors duration-300" title="Toggle Status">
                                        ...
                                        <form method="POST" action="{{ route('admin.blogs.destroy', $blog) }}" 
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-400 hover:text-red-300 transition-colors duration-300" title="Delete">
