...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-gradient-to-br from-[#071026] to-[#0b1224] border border-cyan-600/20 p-6 rounded-lg shadow-xl transform hover:scale-[1.01] transition-transform reveal">
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-xs text-cyan-300 uppercase tracking-widest">Projects</div>
                    <div class="stat-count text-4xl font-extrabold text-white mt-2" data-target="{{ $stats['projects'] ?? 0 }}">0</div>
                </div>
                <div class="text-cyan-400 text-3xl opacity-80"><i class="fas fa-project-diagram"></i></div>
            </div>
            <div class="text-sm text-cyan-200 mt-3">Recent: <span class="text-white">{{ $recent_projects->first()->title ?? '—' }}</span></div>
        </div>

        <div class="bg-gradient-to-br from-[#071026] to-[#0b1224] border border-fuchsia-600/20 p-6 rounded-lg shadow-xl transform hover:scale-[1.01] transition-transform reveal">
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-xs text-fuchsia-300 uppercase tracking-widest">Skills</div>
                    <div class="stat-count text-4xl font-extrabold text-white mt-2" data-target="{{ $stats['skills'] ?? 0 }}">0</div>
                </div>
                <div class="text-fuchsia-400 text-3xl opacity-80"><i class="fas fa-microchip"></i></div>
            </div>
            <div class="text-sm text-fuchsia-200 mt-3">Visible: <span class="text-white">{{ $stats['skills'] ?? 0 }}</span></div>
        </div>

        <div class="bg-gradient-to-br from-[#071026] to-[#0b1224] border border-emerald-500/20 p-6 rounded-lg shadow-xl transform hover:scale-[1.01] transition-transform reveal">
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-xs text-emerald-300 uppercase tracking-widest">Contacts (Unread)</div>
                    <div class="stat-count text-4xl font-extrabold text-white mt-2" data-target="{{ $stats['unread_contacts'] ?? 0 }}">0</div>
                </div>
                <div class="text-emerald-400 text-3xl opacity-80"><i class="fas fa-envelope-open-text"></i></div>
            </div>
            <div class="text-sm text-emerald-200 mt-3">Total: <span class="text-white">{{ $stats['total_contacts'] ?? 0 }}</span></div>
        </div>

        <div class="bg-gradient-to-br from-[#071026] to-[#0b1224] border border-purple-600/20 p-6 rounded-lg shadow-xl transform hover:scale-[1.01] transition-transform reveal">
            <div class="flex items-center justify-between">
                <div>
                    <div class="text-xs text-purple-300 uppercase tracking-widest">Blog Posts</div>
                    <div class="stat-count text-4xl font-extrabold text-white mt-2" data-target="{{ $stats['blogs'] ?? 0 }}">0</div>
                </div>
                <div class="text-purple-400 text-3xl opacity-80"><i class="fas fa-blog"></i></div>
            </div>
            <div class="text-sm text-purple-200 mt-3">Published: <span class="text-white">{{ $stats['published_blogs'] ?? 0 }}</span></div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2 bg-[#071026] border border-cyan-600/10 rounded-lg shadow-lg p-6 reveal">
            ...
            <div class="space-y-3">
                @forelse($recent_projects as $p)
                    <a href="{{ route('admin.projects.show', $p) }}" class="block p-3 rounded-md bg-gradient-to-r from-cyan-900/10 to-transparent hover:from-cyan-900/20 transition-colors">
                        ...
                        <div class="text-sm text-cyan-200 mt-1">{{ \Illuminate\Support\Str::limit($p->description ?? $p->title ?? '', 140) }}</div>
                    </a>
                @empty
        ...
        <div class="bg-[#071026] border border-purple-700/10 rounded-lg shadow-lg p-6 reveal">
            ...
            <div class="space-y-3">
                @forelse($recent_blogs ?? [] as $blog)
                    <a href="{{ route('admin.blogs.show', $blog) }}" class="block p-3 rounded-md bg-gradient-to-r from-purple-900/10 to-transparent hover:from-purple-900/20 transition-colors">
                        <div class="flex justify-between items-center">
                        ...
                        <div class="text-sm text-purple-200 mt-1">{{ \Illuminate\Support\Str::limit($blog->excerpt ?? $blog->title ?? '', 140) }}</div>
                        <div class="flex items-center gap-2 mt-2">
                            <span class="px-2 py-1 text-xs rounded-full {{ $blog->status === 'published' ? 'bg-green-500/20 text-green-300' : ($blog->status === 'draft' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-gray-500/20 text-gray-300') }}">
    ...
    <div class="space-y-8">
        ...
        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 reveal">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <!-- Projects Management -->
                <div class="bg-gradient-to-br from-cyan-500/10 to-cyan-600/10 border border-cyan-500/20 rounded-xl p-4 hover:bg-cyan-500/20 transition-all duration-300">
                ...
                <!-- Blog Management -->
                <div class="bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/20 rounded-xl p-4 hover:bg-purple-500/20 transition-all duration-300">
                    <div class="flex items-center gap-3 mb-3">
                ...
                <!-- Skills Management -->
                <div class="bg-gradient-to-br from-fuchsia-500/10 to-fuchsia-600/10 border border-fuchsia-500/20 rounded-xl p-4 hover:bg-fuchsia-500/20 transition-all duration-300">
                    <div class="flex items-center gap-3 mb-3">
                    ...
                    <div class="space-y-2">
                        <a href="#" 
                           class="block w-full px-4 py-2 bg-fuchsia-600/50 text-fuchsia-300 text-sm font-semibold rounded-lg cursor-not-allowed text-center opacity-50">
                            Coming Soon
                        </a>
                        <span class="block w-full px-4 py-2 bg-fuchsia-500/20 border border-fuchsia-400/30 text-fuchsia-300 text-sm font-semibold rounded-lg text-center">
                            Skills Management
                        </span>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-br from-emerald-500/10 to-emerald-600/10 border border-emerald-500/20 rounded-xl p-4 hover:bg-emerald-500/20 transition-all duration-300">
                    <div class="flex items-center gap-3 mb-3">
                    ...
                    <div class="space-y-2">
                        <a href="#" 
                           class="block w-full px-4 py-2 bg-emerald-600/50 text-emerald-300 text-sm font-semibold rounded-lg cursor-not-allowed text-center opacity-50">
                            Coming Soon
                        </a>
                        <span class="block w-full px-4 py-2 bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 text-sm font-semibold rounded-lg text-center">
                            Experience Management
                        </span>
                    </div>
                </div>
        ...
        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 reveal">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <!-- Settings Management -->
                <div class="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-xl p-4 hover:bg-blue-500/20 transition-all duration-300">
                    <div class="flex items-center gap-3 mb-3">
                ...
                <!-- Contact Management -->
                <div class="bg-gradient-to-br from-orange-500/10 to-orange-600/10 border border-orange-500/20 rounded-xl p-4 hover:bg-orange-500/20 transition-all duration-300">
                    <div class="flex items-center gap-3 mb-3">
                ...
                <!-- Dashboard Overview -->
                <div class="bg-gradient-to-br from-indigo-500/10 to-indigo-600/10 border border-indigo-500/20 rounded-xl p-4 hover:bg-indigo-500/20 transition-all duration-300">
                    <div class="flex items-center gap-3 mb-3">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4 pt-8 border-t border-white/10">
        ...
        <div class="flex gap-3">
            <a href="{{ route('home') }}" class="px-4 py-2 rounded-md bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-semibold hover:opacity-90 transition-opacity">
                <i class="fas fa-external-link-alt mr-2"></i>
                View Site
            </a>
            <a href="{{ route('admin.settings.index') }}" class="px-4 py-2 rounded-md bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold hover:opacity-90 transition-opacity">
