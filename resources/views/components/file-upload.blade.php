...
<div class="file-upload-component" data-type="{{ $type ?? 'image' }}" data-folder="{{ $folder ?? 'portfolio' }}">
    ...
    <div class="upload-area bg-white/5 backdrop-blur-sm border-2 border-dashed border-cyan-400/30 rounded-2xl p-8 text-center hover:border-cyan-400/50 transition-all duration-300 cursor-pointer" 
        ...
        <!-- Hidden File Input -->
        <input type="file" 
               id="file-input-{{ $type ?? 'image' }}" 
               name="file" 
               accept="{{ ($type ?? 'image') === 'image' ? 'image/*' : '.pdf,.doc,.docx,.txt' }}"
               class="hidden">
    </div>
    ...
    <!-- Error Messages -->
    <div id="upload-error-{{ $type ?? 'image' }}" class="hidden mt-4 p-4 bg-red-500/20 border border-red-500/30 rounded-xl">
    ...
    <!-- Success Messages -->
    <div id="upload-success-{{ $type ?? 'image' }}" class="hidden mt-4 p-4 bg-green-500/20 border border-green-500/30 rounded-xl">
        <div class="flex items-center gap-2">
